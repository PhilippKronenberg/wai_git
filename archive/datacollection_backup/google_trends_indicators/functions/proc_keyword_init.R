...
proc_keyword_init <- function(keyword = "Insolvenz",
  ...
  # allow customizing wait/retry via environment variables to handle rate limits
  wait_val <- as.integer(Sys.getenv("WAI_WAIT", unset = "20"))
  retry_val <- as.integer(Sys.getenv("WAI_RETRY", unset = "10"))
  message(sprintf("Using wait=%s retry=%s for Google Trends requests", wait_val, retry_val))
